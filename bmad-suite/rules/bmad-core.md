...
例如"实现这个架构设计"应从 Architect 门禁开始，然后切换到 Developer，而不是只按"架构"关键词停留在 Architect。
...
流程：Product/Architect 模式澄清 → **架构门禁 (Preflight)** → 建立 `brain/task.md` → 生成 Epic/Story/Task 实施计划 → **等待用户批准** → Developer 模式分步实现 → 运行测试 → **状态对账 (Reconciliation)** → 切换 Reviewer 模式对抗式审查 → 标记完成。

### L 级任务

全新大模块、重大架构重构、数据库迁移、安全与部署变更。

流程：完整 Spec / 架构基线建立 → **架构门禁** → 实施计划写入 `_bmad-output/specs/` → 拆解 Task 列表 → **等待用户批准** → 逐项编码与验证 → 架构重检 → 独立 Code Review → **状态对账** → 集成部署验证。
